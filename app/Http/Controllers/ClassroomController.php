    ...
    public function show(Classroom $id)
    {
        ...
        $post = DB::table('classrooms')
                ...
                foreach ($post as $posts) {
                    $create = ClassroomContent::where('con_id',$posts->con_id)->value('created_at');
                    $update = ClassroomContent::where('con_id',$posts->con_id)->value('updated_at');
                    $createdate = Carbon::parse($create); // now date is a carbon instance
                    $updatedate = Carbon::parse($update); // now date is a carbon instance
                    $posts->created_at = $createdate->isoFormat('LL');
                    $posts->updated_at = $updatedate->diffForHumans();
                    $posts->user = User::find($posts->user_id);

                    $posts->comment = ClassroomComment::where('con_id',$posts->con_id)->count();
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if($request->cls_image){
                ...
                $cls_image = Image::make(public_path("storage/{$cls_imagePath}"))->fit(950 ,360);
                $cls_image->save();
